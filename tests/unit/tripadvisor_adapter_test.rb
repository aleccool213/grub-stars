...
class TripAdvisorAdapterTest < Minitest::Test
  ...
  def test_search_all_businesses
    ...
    stub_location_details_requests(count: 10)
  ...
  def test_search_all_businesses_fetches_detailed_data
    # Mock search response with 2 locations (no GPS in search results)
    stub_request(:get, "https://api.content.tripadvisor.com/api/v1/location/search")
      .with(query: hash_including(key: "test_tripadvisor_api_key"))
      .to_return(
        status: 200,
        body: {
          data: [
            { location_id: "111", name: "Restaurant A", rating: "4.5", num_reviews: "100" },
            { location_id: "222", name: "Restaurant B", rating: "4.0", num_reviews: "50" }
          ]
        }.to_json,
        headers: { "Content-Type" => "application/json" }
      )

    # Mock detail responses with GPS coordinates
    stub_request(:get, "https://api.content.tripadvisor.com/api/v1/location/111/details")
      .with(query: hash_including(key: "test_tripadvisor_api_key"))
      .to_return(
        status: 200,
        body: {
          ...
          name: "Restaurant A",
          phone: "+1234567890",
          address_obj: { latitude: "21.309971", longitude: "-157.810226" },
          category: { name: "Seafood" }
        }.to_json,
        headers: { "Content-Type" => "application/json" }
      )

    stub_request(:get, "https://api.content.tripadvisor.com/api/v1/location/222/details")
      .with(query: hash_including(key: "test_tripadvisor_api_key"))
      .to_return(
        status: 200,
        body: {
          ...
          name: "Restaurant B",
          phone: "+0987654321",
          address_obj: { latitude: "21.315000", longitude: "-157.820000" },
    ...
    results = []
    @adapter.search_all_businesses(location: "Honolulu, HI") do |biz, _progress|
      results << biz
    end

    # Assert that GPS coordinates are present from detail fetch
    ...
    assert_includes results[0][:categories], "Seafood"

    assert_equal 21.315000, results[1][:latitude]
    assert_equal(-157.820000, results[1][:longitude])
    assert_equal "+0987654321", results[1][:phone]
  end
  ...
  def test_search_all_businesses_handles_detail_fetch_failure
    ...
    stub_request(:get, "https://api.content.tripadvisor.com/api/v1/location/search")
      ...
      .to_return(
        ...
        body: {
          data: [
            { location_id: "333", name: "Restaurant C", rating: "4.0", num_reviews: "30" }
          ]
        }.to_json,
        headers: { "Content-Type" => "application/json" }
      )

    # Mock detail fetch failure (404)
    stub_request(:get, "https://api.content.tripadvisor.com/api/v1/location/333/details")
      .with(query: hash_including(key: "test_tripadvisor_api_key"))
      .to_return(status: 404, body: { message: "Not found" }.to_json)

    # Should not raise, should continue with basic data
    results = []
    @adapter.search_all_businesses(location: "test location") do |biz, _progress|
      results << biz
    end

    assert_equal 1, results.length
    assert_equal "Restaurant C", results[0][:name]
    # GPS will be nil since detail fetch failed
    assert_nil results[0][:latitude]
    assert_nil results[0][:longitude]
  end

  def test_search_all_businesses_skip_details_when_disabled
    stub_location_search_request(count: 3)

    # Should NOT make detail requests when fetch_details: false
    results = []
    @adapter.search_all_businesses(location: "toronto", fetch_details: false) do |biz, _progress|
      results << biz
    end

    assert_equal 3, results.length
    # No detail stubs, so if it tried to fetch details it would fail
    # Since we didn't stub detail endpoints, reaching here means they weren't called
  end
  ...
  def stub_location_details_request(location_id)
    stub_request(:get, "https://api.content.tripadvisor.com/api/v1/location/#{location_id}/details")
      .with(query: hash_including(key: "test_tripadvisor_api_key"))
      .to_return(
        status: 200,
        body: {
  ...
  def stub_location_details_requests(count:)
    # Stub detail requests for each location_id generated by stub_location_search_request
    count.times do |i|
      location_id = (123456 + i).to_s
      stub_request(:get, "https://api.content.tripadvisor.com/api/v1/location/#{location_id}/details")
        .with(query: hash_including(key: "test_tripadvisor_api_key"))
        .to_return(
          ...
          body: {
            location_id: location_id,
            name: "Restaurant #{i}",
            phone: "+1-555-000-#{i.to_s.rjust(4, '0')}",
            address_obj: {
              latitude: (44.0 + i * 0.01).to_s,
              longitude: (-79.0 - i * 0.01).to_s
            },
