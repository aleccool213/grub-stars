...
# Bug: Missing TripAdvisor Ratings After Restaurant Merge

## Summary
After merging duplicate restaurant records for "Homestead Artisan Bakery", the TripAdvisor rating is not displaying on the restaurant details page. Only the Google rating is visible.

**Expected behavior:** Restaurant details page should show ratings from all sources: Google, Yelp, and TripAdvisor.

**Actual behavior:** Only Google rating (4.6 stars) is displayed. TripAdvisor and Yelp ratings are missing.

## Root Cause
When manually merging the duplicate restaurants via SQL, the ratings from the deleted restaurants (Yelp and TripAdvisor) were not moved to the surviving restaurant record (Google).

**Database state after merge:**
- Restaurant #3 (Google - surviving): Has Google rating only
- Restaurant #4 (Yelp - deleted): Had Yelp rating (not moved before delete)
- Restaurant #5 (TripAdvisor - deleted): Had TripAdvisor rating (not moved before delete)

## Affected Data
- `ratings` table - Missing Yelp and TripAdvisor entries for restaurant_id 3
- Restaurant details page - Only shows Google rating

## Steps to Reproduce
1. Navigate to Homestead Artisan Bakery details page
2. Observe: Only Google rating (4.6 stars, 712 reviews) is displayed
3. Expected: Should also show Yelp and TripAdvisor ratings

## Suggested Fix

**Option 1 - Re-index the restaurant (recommended):**
```bash
# Re-run indexing for this specific restaurant to fetch fresh data
grst index --location "Barrie, Ontario" --name "Homestead Artisan Bakery"
```

**Option 2 - Manually insert missing ratings:**
```sql
-- Insert Yelp rating (if we know the values)
INSERT INTO ratings (restaurant_id, source, score, review_count, fetched_at)
VALUES (3, 'yelp', 3.6, 13, datetime('now'));

-- Insert TripAdvisor rating (need to fetch from API or re-index)
INSERT INTO ratings (restaurant_id, source, score, review_count, fetched_at)
VALUES (3, 'tripadvisor', 4.5, 45, datetime('now'));
```

**Option 3 - Fix the merge process:**
Update the manual merge SQL to include moving ratings:
```sql
-- When merging, always move ratings first
UPDATE ratings SET restaurant_id = 3 WHERE restaurant_id IN (4, 5);
-- Then update external_ids
-- Then delete duplicates
```

## Prevention
Add to `docs/deduplication-solutions.md`:
- Always move ratings, reviews, and media before deleting duplicate restaurants
- Create a proper merge utility/script instead of manual SQL

## Files to Review
- `docs/deduplication-solutions.md` - Merge procedures
- `lib/services/index_restaurants_service.rb` - Re-indexing logic
